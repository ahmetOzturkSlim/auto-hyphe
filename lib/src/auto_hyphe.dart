...
class AutoHyphe extends StatelessWidget {
  const AutoHyphe(
    this.text, {
    this.maxWidthOfLine = 1,
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      ...
      for (int i = 0; i < words.length; i++) {
        ...
        if (currentLineSpaceUsed + wordWidth < constraints.maxWidth * maxWidthOfLine - endBuffer) {
          ...
          final int? syllableToUse = words[i].length == 1
              ? null
              : getLastSyllableIndex(syllables, constraints.maxWidth * maxWidthOfLine - currentLineSpaceUsed, effectiveTextStyle, lines);

          if (syllableToUse == null || (shouldHyphenate != null && !shouldHyphenate!(constraints.maxWidth * maxWidthOfLine, currentLineSpaceUsed, wordWidth))) {
        ...
        if (i != words.length - 1) {
          if (currentLineSpaceUsed + singleSpaceWidth < constraints.maxWidth * maxWidthOfLine) {
            texts.add(const TextSpan(text: " "));
